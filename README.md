...
Enabled installed plugins are bootstrapped as framed stdio runtimes. Their `plugin.register` actions and views are merged into the same runtime catalog as bundled features, so `actions` and `run <plugin_action>` work for external plugins as well.
